...
class AgenticCronRunner:
    ...
    def _run_task(self, name: str, state: dict[str, Any]) -> dict[str, Any]:
        if name == "dream":
            base = self.maintenance.run_dream_cycle(idle_minutes=int(state.get("dream_idle_minutes", 30)))
            # Always emit a structured record to logs/dream.log so the
            # Command Center has something to live-tail. Skip the donor call
            # if the maintenance idle gate said "not yet".
            try:
                from dream_engine import run_dream_once, _append_dream  # type: ignore
                if base.get("dream_ran"):
                    base["reflection"] = run_dream_once(trigger="cron")
                else:
                    _append_dream(
                        {
                            "ts": time.time(),
                            ...
                            "ok": True,
                            "skipped_reason": "idle_threshold_not_met",
                            "idle_minutes": base.get("idle_minutes"),
                        }
                    )
            except Exception as exc:  # pragma: no cover
                base["dream_engine_error"] = str(exc)
            return base
