...
class GatorMemoryCore:
    ...
    def _open_or_create_scratchpad(self, dim: int = SCRATCHPAD_DEFAULT_DIM):
        ...
        try:
            ...
            # Touch the table to force manifest validation. If LanceDB's manifests
            # are corrupted (0-byte or unreadable), this raises; we then self-heal
            # by wiping the transient table directory and recreating it.
            try:
                _ = table.schema
            except Exception:
                raise
            ...
            import shutil
            target = self.db_path / f"{SCRATCHPAD_TABLE}.lance"
            try:
                if target.exists():
