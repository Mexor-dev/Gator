...
def terminate_stack() -> None:
    """Best-effort cleanup of wakeup-managed processes from pid files."""
...
def memory_check() -> dict:
    before = int(
        subprocess.check_output(
            [str(GATOR_ROOT / "venv/bin/python"), str(MEMORY_CORE), "--count"], text=True
        ).strip()
    )
    ...
    ingest_cmd = [str(GATOR_ROOT / "venv/bin/python"), str(MEMORY_CORE), "--ingest", dummy]
    ingest_raw = subprocess.check_output(ingest_cmd, text=True)
...
def main() -> None:
    ...
    wakeup_proc = subprocess.Popen(["bash", str(WAKEUP)], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
