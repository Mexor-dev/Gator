...
_GENERIC_PHRASE_REPLACEMENTS: list[tuple[str, str]] = [
    ...
    # NOTE: previously this rule rewrote "understood" -> "Task acknowledged. ",
    # which combined with the next rule below produced the
    # "Task acknowledged. Moving to execution." stutter loop. Replaced with a
    # neutral confirmation that does not match FORBIDDEN_TEMPLATES.
    (r"(?i)^\s*understood\b[\s,.:;-]*", "Confirmed. "),
    (r"(?i)\bright\s+away\b", "now"),
...
class PersonaEngine:
    ...
    def refine_response(self, text: str, *, user_text: str = "", scratchpad: str = "") -> str:
        ...
        # Loop-breaker: do NOT auto-append "Moving to execution." when the
        # response already starts with the canonical acknowledgement. The
        # Dual-Core controller in gator_bridge owns acknowledgement phrasing
        # via the Reasoning Trace; persona_engine must not synthesise it here.
