...
class TelegramGateway:
    ...
    async def text_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            # Native gator-server (35B weights) not yet healthy. Reply with a
            # human-friendly warming-up message instead of leaking [Errno 111]
            # / 502 / ConnectionRefused traces to the user.
            print(f"[WARN] Bridge URLError -> warming-up reply: {exc}", flush=True)
            await self._send_text_and_voice(
                context.bot,
                update.effective_chat.id,
                "Systems are still warming up. Please wait 60 seconds while I seat the 35B weights.",
            )
            return
        except OSError as exc:
            # ConnectionRefusedError ([Errno 111]) is an OSError subclass on
            # some Python builds. Mirror the warming-up reply instead of
            # surfacing the raw socket error.
            if getattr(exc, "errno", None) in (111, 104, 110):
                print(f"[WARN] Bridge socket error -> warming-up reply: {exc}", flush=True)
                await self._send_text_and_voice(
                    context.bot,
                    update.effective_chat.id,
                    "Systems are still warming up. Please wait 60 seconds while I seat the 35B weights.",
