...
class TelegramHiveGateway:
    ...
    async def text_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not text:
            return
        target, task = self._resolve_target(text)
        url = self._route_bridge(target)
        payload = {"prompt": task, "max_tokens": 350, "temperature": 0.7, "top_k": 40}
        try:
            out = await asyncio.to_thread(_post_json, url, payload, 120.0)
            answer = str(out.get("text") or out.get("output") or out.get("response") or "").strip()
            ...
            if not validation.get("ok", False):
                await context.bot.send_message(
                    ...
                    text=f"[{target}] Gator-Guard blocked an invalid numeric claim; response withheld until verified context is available.",
                )
                return
            await context.bot.send_message(chat_id=update.effective_chat.id, text=f"[{target}] {answer}")
        except Exception as exc:
            await context.bot.send_message(chat_id=update.effective_chat.id, text=f"[{target}] error: {exc}")
        ...
        while True:
            try:
                ...
                for packet in batch.get("packets", []):
                    ...
                    msg = str(packet.get("text") or "").strip()
                    if msg:
                        await app.bot.send_message(chat_id=int(self.auth_chat_id), text=f"[{speaker}] {msg}")
            except Exception:
                pass
            await asyncio.sleep(0.8)
