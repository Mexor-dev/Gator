...
class MitosisEngine:
    ...
    def _persona_contract(self) -> dict[str, str]:
        return {
            "iron_gator": os.environ.get("GATOR_IRON_GATOR", "true"),
            "persona_lineage": os.environ.get("GATOR_PERSONA_LINEAGE", "telegram-iron-gator-v1"),
            "persona_source": os.environ.get(
                "GATOR_PERSONA_SOURCE",
                "telegram_gateway_validated_prompts",
            ),
        }
    ...
    def spawn_clone(self, name: str) -> dict[str, Any]:
        ...
        persona_contract = self._persona_contract()
        ...
        # Iron-Gator persona contract: every clone inherits the same Telegram
        # gateway-validated MOUTHPIECE_PROMPT + SYSTEM_TRACE_PROMPT used by the
        # successful Telegram build. The bridge reads this flag and keeps the
        # commitment-tone hidden hint pinned in the donor system prompt.
        env["GATOR_IRON_GATOR"] = persona_contract["iron_gator"]
        env["GATOR_PERSONA_LINEAGE"] = persona_contract["persona_lineage"]
        env["GATOR_PERSONA_SOURCE"] = persona_contract["persona_source"]
