...
## Operational Targets (Iron-Gator Release Standard — May 2026)

| Metric                          | Target                |
|---------------------------------|-----------------------|
| Steady-state VRAM (RTX 3050 6GB)| **≤ 2.1 GB** (live: 2076 MiB) |
| Time-to-first-token (TTFT)      | **< 100 ms** (live: 60 ms) |
| Logic graft density             | **≥ 3,500 records** (live: 3,672 / 2.47 MB) |
| RAM swap during 1k-ctx infer    | **0 bytes**           |
| Cold ignition → first reply     | < 12 s                |
| Donor process at runtime        | **decommissioned**    |
...
Installed by `installer.sh` to `~/.local/bin/GATOR_CTL`.

| Command                                                          | Effect                                                  |
|------------------------------------------------------------------|---------------------------------------------------------|
| `GATOR_CTL status`                                               | bridge + server health, VRAM, gate record count         |
| `GATOR_CTL health`                                               | quick HTTP check                                        |
| `GATOR_CTL vram`                                                 | live VRAM vs guard threshold                            |
| `GATOR_CTL ttft`                                                 | measure first-token latency                             |
| `GATOR_CTL revert`                                               | atomic swap `logic_map.gate ← .prev`                    |
| `GATOR_CTL abort --timeout 5`                                    | SIGTERM bridge, escalate to SIGKILL after Nms, revert   |
| `GATOR_CTL --engine-abort --revert-gate-prev --timeout 5ms`      | flat-flag form (Technical-Manual style)                 |

---

## Zero-Touch Install (Iron-Gator)

```bash
chmod +x installer.sh
./installer.sh -y          # VRAM-profile, lock deps, place binaries, verify gate
python3 bootstrap.py       # 4-phase Brain-Start (kernel → gate → bridge → chassis)
```

`installer.sh` enforces:
* **VRAM gate** — < 4 GB total VRAM → 1.5B chassis is locked, 35B donor blocked
* **Dependency lock** — pinned to validated `requirements.txt`
* **Binary placement** — `libgator_kern.so → /usr/local/lib/`, `GATOR_CTL → ~/.local/bin/`
* **Gate hygiene** — refuses to start if `bin/logic_map.gate` has < 3,500 records

`bootstrap.py` runs four phases in strict order; any failure aborts and rolls back the bridge:
1. **Kernel Map** — `dlopen` `libgator_kern.so`, verify exported symbols
2. **Gate Loading** — load `logic_map.gate`; on corruption, self-heal to `.prev`
3. **Bridge Ignition** — spawn GatorBridge with Cold-Path Boost (+1.5) and VRAM Guard (2.2 GB)
4. **Chassis Link** — 1.5B Deep-Logic self-test (must articulate kernel ⇄ gate relationship)
